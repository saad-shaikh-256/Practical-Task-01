...
const StudentList = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // For buttons (Add/Edit/Delete)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalType === "add") {
        ...
        await axios.put(
          `${API_URL}/updateUser/${selectedStudent._id}`,
          formData
        );
      }
      await fetchStudents(); // Wait for list to refresh
      closeModal();
    } catch (err) {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: students.length > 0 ? students.length - 1 : 0,
      icon: "user-check-01",
      color: "text-white",
      bg: "bg-zinc-900",
      border: "border-zinc-700",
    ...
    <div className="flex-1 h-full flex flex-col p-6 lg:p-10 overflow-hidden relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-xl flex items-center gap-4"
          >
            <div
              className={`w-12 h-12 rounded-lg flex items-center justify-center text-xl ${stat.bg} ${stat.color} border ${stat.border}`}
            >
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm divide-y divide-zinc-800/50">
              ...
              {students.map((student) => (
                <tr
                  key={student._id}
                  className="group hover:bg-zinc-900/40 transition-colors duration-150"
                >
                  ...
                  <td className="p-5">
                    <span className="bg-zinc-900 border border-zinc-700 text-zinc-300 px-2.5 py-1 rounded text-xs font-medium">
                      {student.course}
                    </span>
                  </td>
                  ...
                  <td className="p-5 text-right pr-6">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      ...
                      <button
                        ...
                        className="p-2 text-zinc-400 hover:text-rose-400 hover:bg-zinc-800 rounded-md transition-all"
                      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              type="text"
              required
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-zinc-100 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-zinc-600 disabled:opacity-50"
              value={formData.name}
              onChange={(e) =>
          ...
          <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-zinc-800">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-lg shadow-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
                <>
                  {/* HugeIcons Loading Spinner */}
                  <i className="hgi hgi-stroke hgi-loading-02 animate-spin text-lg"></i>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 mt-2">
            ...
            <button
              ...
              disabled={isSubmitting}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-rose-600 hover:bg-rose-700 text-white rounded-lg shadow-lg shadow-rose-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
                <>
                  <i className="hgi hgi-stroke hgi-loading-02 animate-spin text-lg"></i>
